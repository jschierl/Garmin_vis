...
class FoliumMap:
    ...
    def load_data(self, gpx_path, state='None', city='None'):
        ...
        for file_path in gpx_files:
            ...
            activity = GarminActivity(time, points)
            if len(points) > 2:
                self.activity_points.append(points)
                self.activities.append(activity)

                # If we're filtering by city or state, append thes to new lists: "_filtered_activities" and "_filtered_activity_points"
                if (not state == 'None' or not city == 'None') and len(self.activities) > 0:
                    filtered = False
                    if not hasattr(self, "_filtered_activities") and not hasattr(self, "_filtered_activity_points"):
                        self._filtered_activities = []
                        self._filtered_activity_points = []
                    if hasattr(activity, "_city") and hasattr(activity, "_state"):
                        ...
                        if state in activity._state or not state:
                            filtered = True
                    if filtered:
                        self._filtered_activity_points.append(points)
                        self._filtered_activities.append(activity)

        print("Loaded.")

    def draw_points(self):
        if hasattr(self, "_filtered_activity_points"):
            activities_to_draw = self._filtered_activity_points
        else:
            activities_to_draw = self.activity_points
        for coords in activities_to_draw:
            folium.PolyLine(coords).add_to(self._map)

            # Add markers for the start and finish points
            start_point = folium.Marker(location=coords[0], popup='Start')
            start_point.add_to(self._map)
            end_point = folium.Marker(location=coords[-1], popup='Finish')
            end_point.add_to(self._map)
        
    def save_map(self, mapName="map"):
        if hasattr(self, "_map"):
            self._map.save('{}.html'.format(mapName))
        else:
            raise ValueError("Map hasn't been initialized yet")
...
if __name__ == "__main__":
    ...
    vis_map = FoliumMap()
    vis_map.load_data(args.gpx_path, state=args.state, city=args.city)
